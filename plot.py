...
cudir   = os.getcwd()
surf_json = os.path.join(cudir, "surf.json")
reac_json = os.path.join(cudir, "reaction_energy.json")
loss_file = os.path.join(cudir, "loss.h5")
eneg_file = os.path.join(cudir, "ped.h5")
...
if runmax == 0:
	runmax = int(df["run"].max())
runmin = int(df["run"].min())
maxval = df["score"].max()
minval = df["score"].min()
...
def plot_bar(runmax, df):
    ...
    figure.update_yaxes(title="Score", title_font_family="Arial", color="black", title_font_size=18, tickwidth=2, ticklen=8,
                        showline=True, showgrid=False, showticklabels=True, linecolor="black", linewidth=2, tickformat="",
                        ticks="outside", tickfont=dict(family="Arial", size=16, color="black"), mirror=True, 
                        range=[minval-0.05*abs(minval), maxval])
    figure.write_image("bar.pdf")

plot_bar(runmax=runmax, df=df)
#
# loss plot
#
...
def plot_violin(runmax, df):
    ...
    for run in range(runmax+1):
        figure.add_trace(go.Violin(x=df_surf["run"][df_surf["run"]==run], y=df_surf["score"][df_surf["run"]==run], 
                                   box=dict(visible=True), points="all", fillcolor=colors[run+1], line_color="black", line_width=1, 
                                   opacity=0.8, name="Run "+str(run)))
...
plot_violin(runmax=runmax, df=df)

def plot_mean_values(runmax, df):
    """
    mean plot
    """
    figure = go.Figure()
    ...
    figure.update_yaxes(title="Mean score", title_font_family="Arial", color="black", title_font_size=18, tickwidth=2, ticklen=8,
                        showline=True, showgrid=False, showticklabels=True, linecolor="black", linewidth=2, tickformat=".2f",
                        ticks="outside", tickfont=dict(family="Arial", size=16, color="black"), mirror=True)
    figure.write_image("mean.pdf")

plot_mean_values(runmax=runmax, df=df)
#
...
for run in runs:
	...
	figure.add_trace(go.Bar(x=x, y=deltaE, marker_line_width=1, marker_color=colors[run+1], marker_line_color="black", 
                            name="Run "+str(run)))
...
def plot_coverage():
    ...
    for run in runs:
	    ...
	    figure.add_trace(go.Bar(x=species, y=coverage, marker_color=colors[run+1], marker_line_width=1, marker_line_color="black", 
                                name="Run "+str(run)))
    ...
    figure.update_xaxes(title="Species", title_font_family="Arial", color="black", title_font_size=18, tickwidth=2, ticklen=8,
    					showline=True, showgrid=False, showticklabels=True, linecolor="black", linewidth=2, tickformat="",
    					ticks="outside", tickfont=dict(family="Arial", size=16, color="black"), mirror=True)
    figure.update_yaxes(title="Fractional coverage (-)", title_font_family="Arial", color="black", title_font_size=18, 
    ...
    if best:
        ...
        figure.update_layout(
    	    				 xaxis = dict(showline=True, showgrid=False, showticklabels=True, linecolor="black", 
                                            linewidth=2, tickformat="", ticks="outside", 
                                            tickfont=dict(family="Arial", size=16, color="black"), mirror=True),
		    			     yaxis = dict(showline=True, showgrid=False, showticklabels=True, linecolor="black", 
                                            linewidth=2, tickformat=".1f", ticks="outside", 
                                           tickfont=dict(family="Arial", size=16, color="black"), mirror=True),
					        plot_bgcolor="white",
					        )

        figure.update_xaxes(title="Steps", title_font_family="Arial", color="black", title_font_size=18, tickwidth=2, ticklen=8)
        figure.update_yaxes(title="Reaction energy (eV)", title_font_family="Arial", color="black", 
                         title_font_size=18, tickwidth=2, ticklen=8, range=[-3,1])
        ...
        figure.update_xaxes(title="Steps", title_font_family="Arial", color="black", title_font_size=18, tickwidth=2, ticklen=8,
           					showline=True, showgrid=False, showticklabels=True, linecolor="black", linewidth=2, tickformat="",
           					ticks="outside", tickfont=dict(family="Arial", size=16, color="black"), mirror=True)
        figure.update_yaxes(title="Potential energy (eV)", title_font_family="Arial", color="black", title_font_size=18, 
                            tickwidth=2, ticklen=8,showline=True, showgrid=False, showticklabels=True, 
                            linecolor="black", linewidth=2, tickformat=".1f",ticks="outside", 
                            tickfont=dict(family="Arial", size=16, color="black"), mirror=True, range=[-4,2])
