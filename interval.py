...
app.layout = html.Div(
    children=[
        ...
        html.Div([
            html.Div([
                dcc.RadioItems(id="coverage-yaxis", options=[{"label": i, "value": i}
															 for i in ["linear", "log"]],
															 value="linear",
...
def display_num(n_intervals, intervals):
    ...
    return html.Div('updated {0:d} times (updating every {1:.0f} min)'.format(n_intervals, intervals/60/1e3),
																						   style=style)
...
#
# plotting currently the best energy diagram
#
@app.callback(Output("potential_energy_diagram", "figure"),
             [Input("interval-component", "n_intervals")])
def make_energy_diagram(_):
...
def make_coverage_bar(_, yaxis_type):
...
@app.callback(Output("reaction_energy", "figure"),
             [Input("interval-component", "n_intervals")])
def make_reaction_energy_bar(_):
    df = pd.read_json(reac_json)
    df = df.set_index("unique_id")
    df = df.sort_values("score", ascending=False)
    ...
    figure.update_layout(margin=dict(l=0, r=40, t=20, b=20),
                         xaxis_title="steps", yaxis_title="Reaction energy (eV)", height=height)
    return figure
